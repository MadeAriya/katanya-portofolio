...
const ChatHeader = ({ changeName }) => {
  return (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pb-4 border-b border-gray-700">
      ...
      <button onClick={changeName} className="p-3 text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-red-500 self-start md:self-center transform hover:scale-105">
        Change Name
      </button>
    </div>
  );
